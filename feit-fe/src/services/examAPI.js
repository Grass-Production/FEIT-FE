...
export const getExamByIdUnit = async (id) => {
    const res = await get(`http://localhost:8080/api/exam/fetch/1/unit_id?unit_id=${id}`, {
        headers: {
            "Content-Type": "application/json",
        },
        credentials: "include",
...
export const getQuestionByIdUnit = async (id) => {
    const res = await get(`http://localhost:8080/api/exam/question/fetch/exam_id?exam_id=${id}`, {
        headers: {
            "Content-Type": "application/json",
        },
...
//             const exam = await getExamByIdUnit('664637576c4c00abf5dee043');
